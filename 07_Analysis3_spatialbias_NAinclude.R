...
# save resTemp_species as R object file
#   NOTE: to be used for the NAexclude analysis to save time
saveRDS(object = resTemp_species, "resTemp_speciesUpdate.rds")
...
pdf("./Figures_unedited/GeneheatmapRawScaleNoBroder_NAinclude.pdf", paper = "a4")
...
pdf("./Figures_unedited/SpeciesheatmapRawScaleNoBorder_NAinclude.pdf", paper = "a4")
...
pdf("./Figures_unedited/geneXspecies_province_NAinclude.pdf", paper = "a4r")
...
pdf("./Figures_unedited/worldBarcodeDist_NAinclude.pdf", paper = "a4r")
...
pdf("./Figures_unedited/worldBarcodeDist_philZoom_NAinclude.pdf", paper = "a4r")
...
pdf("./Figures_unedited/geneXyearCollect_NAinclude.pdf", paper = "a4r")
...
#   NOTE: include smoothened curves obtained via loess regression
pdf("./Figures_unedited/geneXyearSubmission_NAinclude.pdf", paper = "a4r")
...
# define data frame of unique provinceUse entries in barcode_data_final
provinceRegInfo <- data.frame(prov = unique(barcode_data_final$provinceUse))

# add new column in provinceRegInfo for editing
provinceRegInfo$region <- NA

# convert class of region column to character
provinceRegInfo$region <- as.character(provinceRegInfo$region)

# assign appropriate PH administrative region per prov entry 
#   NOTE: based on PSGC database by Philippine Statistics Authority but
#   ARMM region is now referred to as BARMM region (Bangsamoro)
provinceRegInfo[grep("ilocos|union|pangasinan",
                     provinceRegInfo$prov), ]$region <- "region 1"

provinceRegInfo[grep("batanes|cagayan|isabela|vizcaya|quirino",
                     provinceRegInfo$prov), ]$region <- "region 2"

provinceRegInfo[grep("bataan|bulacan|ecija|pampanga|tarlac|zambales|aurora",
                     provinceRegInfo$prov), ]$region <- "region 3"

provinceRegInfo[grep("batangas|cavite|laguna|quezon|rizal",
                     provinceRegInfo$prov),]$region <- "region 4a"

provinceRegInfo[grep("marinduque|palawan|mindoro|romblon",
                     provinceRegInfo$prov),]$region <- "region 4b"

provinceRegInfo[grep("albay|camarines|catanduanes|masbate|sorsogon",
                     provinceRegInfo$prov),]$region <- "region 5"

provinceRegInfo[grep("aklan|antique|capiz|iloilo|negros occidental|guimaras",
                     provinceRegInfo$prov),]$region <- "region 6"

provinceRegInfo[grep("bohol|cebu|negros oriental|siquijor",
                     provinceRegInfo$prov),]$region <- "region 7"

provinceRegInfo[grep("samar|leyte|biliran",
                     provinceRegInfo$prov),]$region <- "region 8"

provinceRegInfo[grep("zamboanga",
                     provinceRegInfo$prov),]$region <- "region 9"

provinceRegInfo[grep("bukidnon|camiguin|lanao del norte|misamis",
                     provinceRegInfo$prov),]$region <- "region 10"

provinceRegInfo[grep("davao|compostela valley",
                     provinceRegInfo$prov),]$region <- "region 11"

provinceRegInfo[grep("cotabato|sultan|sarangani",
                     provinceRegInfo$prov),]$region <- "region 12"

provinceRegInfo[grep("agusan|surigao|dinagat",
                     provinceRegInfo$prov),]$region <- "region 13"

provinceRegInfo[grep("metro manila",
                     provinceRegInfo$prov),]$region <- "metro manila"

provinceRegInfo[grep("abra|benguet|ifugao|kalinga|mountain|apayao",
                     provinceRegInfo$prov),]$region <- "car"

provinceRegInfo[grep("basilan|lanao del sur|maguindanao|sulu|tawi",
                     provinceRegInfo$prov),]$region <- "armm"

# remove row that is NA for prov
provinceRegInfo <- provinceRegInfo[!is.na(provinceRegInfo$prov), ]

# write output for data frame of PH province and region info
#   NOTE: to be used for the NAexclude analysis
fwrite(provinceRegInfo, 
       "~/Desktop/THESIS/Bio 200/Working Files/provinceRegionInfo.table", 
       sep = "\t")
...
pdf("./Figures_unedited/refmapRegion_NAinclude.pdf", paper = "a4")
...
pdf("./Figures_unedited/sourceXprocess_matrix_NAinclude.pdf", paper = "a4r")

plot(rastTemp, xaxt = "n", yaxt = "n", box = FALSE, axes= FALSE)
...
# write output of updated working barcode data table
#   NOTE: to be used for NAexclude analysis 
fwrite(barcode_data_final, 
       "UPDATEworking_barcode_data_NAinclude.table", 
       sep = "\t", 
       na = NA)
